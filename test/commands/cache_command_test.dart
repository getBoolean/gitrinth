...
void main() {
  ...
  setUp(() {
    ...
    env = {...Platform.environment, 'GITRINTH_CACHE': cacheRoot.path};
  });
      ...
      'files': [
        {
          ...
          'filename': filename,
          'hashes': {'sha1': '0' * 40, 'sha512': sha512Hex(jarBytes)},
  ...
  test(
    'cache list emits parseable JSON with seeded modrinth + url artifacts',
    () async {
      ...
      expect(artifacts.where((a) => a['filename'] == 'version.json'), isEmpty);
    },
  );

  test(
    'cache clean --force wipes content but preserves the root dir',
    () async {
      seedModrinthArtifact(
        projectId: 'PROJ1',
        versionId: 'VER1',
  ...
  test(
    'cache repair on a tampered url-cached file deletes it, exits 0',
    () async {
      ...
      expect(
        Directory(
          p.join(cacheRoot.path, 'url', expected.substring(0, 2)),
        ).existsSync(),
        isFalse,
      );
    },
